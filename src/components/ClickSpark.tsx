...
const ClickSpark = ({
  ...
  const spawnSparksAt = (clientX: number, clientY: number) => {
    ...
    // Only draw if click falls within wrapper bounds
  ...
  useEffect(() => {
    if (!listenOnDocument) return;
    const handler = (e: MouseEvent | globalThis.MouseEvent) => {
      spawnSparksAt((e as MouseEvent).clientX, (e as MouseEvent).clientY);
    };
    document.addEventListener('click', handler as EventListener, { passive: true } as AddEventListenerOptions);
    return () => {
  ...
  return (
    <div
      ...
      onClick={listenOnDocument ? undefined : handleClick}
    >
      {children}
      <canvas
        ref={canvasRef}
